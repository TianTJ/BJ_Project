...
function create_box(map,star_point,end_point) {
    $('#box').empty()
    let box_idx = 0
    let p_x = 0;0
    let p_y=0
    for(let i in map){
        for(let j in map[i]){
            if(map[i][j] == 0){
                $('#box').append("<div class='box_hinder box_"+box_idx+"'>")
                $('#box .box_'+box_idx).css({'background-color':"#34c2e3"})
            }else if(map[i][j] == 1){
                //障碍物
                $('#box').append("<div class='box_hinder box_"+box_idx+"'>")
                $('#box .box_'+box_idx).css({'background-color':"#cee366"})
            }else if(map[i][j] == 8){
                $('#box').append("<div class='box_hinder box_"+box_idx+"'>")
                $('#box .box_'+box_idx).css({'background-color':"#FF0000"})
            }
            if(i==star_point.x&&j==star_point.y){
                //起点
                $('#box').append("<div class='box_hinder box_"+box_idx+"'>")
                $('#box .box_'+box_idx).css({'background-color':"#FFA19C"})
            }
            if(i==end_point.x&&j==end_point.y){
                //终点
                $('#box').append("<div class='box_hinder box_"+box_idx+"'>")
                $('#box .box_'+box_idx).css({'background-color':"#E277FF"})
            }
            $('#box .box_'+box_idx).css({'position':'absolute','left':p_x,'top':p_y})
            p_y+=74;
            box_idx++
        }
        p_x+=74
